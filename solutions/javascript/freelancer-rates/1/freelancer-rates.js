...
export function daysInBudget(budget, ratePerHour) {
  var numberOfDays = budget / dayRate(ratePerHour);
...
export function priceWithMonthlyDiscount(ratePerHour, numDays, discount) {
  ...
  const monthlyCost = 22 * dayRate(ratePerHour);
  const discountedMonthlyCost = monthlyCost * (1 - discount);
  const total = fullMonths * discountedMonthlyCost + extraDays * dayRate(ratePerHour);
